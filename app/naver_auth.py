...
from app.models import NaverUserInfo

class NaverAuthService:
    @staticmethod
    def get_naver_auth_url(state: Optional[str] = None) -> str:
        """네이버 로그인 URL 생성"""
    ...
    @staticmethod
    async def get_naver_tokens(code: str, state: str) -> Dict[str, Any]:
        """네이버 서버에서 토큰 받기"""
        try:
            token_data = {
    ...
    @staticmethod
    async def get_naver_user_info(access_token: str) -> NaverUserInfo:
        """네이버 사용자 정보 가져오기"""
        try:
            headers = {
    ...
    @staticmethod
    async def revoke_naver_token(access_token: str) -> bool:
        """네이버 토큰 무효화"""
        try:
            params = {
                "grant_type": "delete",
