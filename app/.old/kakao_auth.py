...
from app.models import KakaoUserInfo

class KakaoAuthService:
    @staticmethod
    def get_kakao_auth_url(state: Optional[str] = None) -> str:
        """카카오 로그인 URL 생성"""
        params = {
    ...
    @staticmethod
    async def get_kakao_tokens(code: str) -> Dict[str, Any]:
        """카카오 서버에서 토큰 받기"""
        try:
            token_data = {
    ...
    @staticmethod
    async def revoke_kakao_token(access_token: str) -> bool:
        """카카오 토큰 무효화 (로그아웃)"""
        try:
            headers = {
                "Authorization": f"Bearer {access_token}",
